...
// // Helper Function: Get AllFileKeys for a user
// func (userdata *User) GetAllFileKeys() (afk AllFileKeys, err error) {
// 	var allfilekeys AllFileKeys
	
// 	// Get marshal
// 	allfilekeys_marshal, ok := userlib.DatastoreGet(userdata.FKKey)
// 	if !ok { return allfilekeys, errors.New("Couldn't get all file keys") }

// 	// Unmarshal
// 	err = json.Unmarshal(allfilekeys_marshal, &allfilekeys)
// 	if err != nil { return allfilekeys, errors.New("Couldn't get all file keys") }
...
// 	// get original filekeys from allfilekeys from Datastore **/
// 	allfilekeys, err = userdata.GetAllFileKeys()
// 	if err != nil {
// 		return filekeys, fileinfo, err
// 	}
// 	filekeys, ok := allfilekeys.Files[filename]
// 	if !ok {
// 		return filekeys, fileinfo, errors.New("File " + filename + " does not exist")
// 	}

// 	// get original fileinfo cipher from Datastore
...
// 	// HMAC key
// 	HMACKey []byte
// 	// location of AllFileKeys
// 	FKKey uuid.UUID
// }

// // maps strings (file names) to File structs
// type AllFileKeys struct {
...
// 	userdata.DSKey = bytesToUUID(userlib.Argon2Key([]byte(password), salt, 16))
// 	userdata.EncryptKey = userlib.Argon2Key([]byte(password), salt, 16)
// 	userdata.HMACKey = userlib.Argon2Key([]byte(password), salt, 16)
// 	userdata.FKKey = bytesToUUID(userlib.Argon2Key([]byte(password), salt, 16))
	
// 	// create and store blank AllFileKeys
// 	var allfilekeys AllFileKeys
// 	allfilekeys.Files = make(map[string]FileKeys)
// 	allfilekeys_marshal, _ := json.Marshal(allfilekeys)
// 	userlib.DatastoreSet(userdata.FKKey, allfilekeys_marshal)
...
// 	// marshal, encrypt, and HMAC the User struct
...
// 	//!!!!!! Now get allfilekeys back
// 	allfilekeys_marshal, _ = userlib.DatastoreGet(userdata.FKKey)
// 	err = json.Unmarshal(allfilekeys_marshal, &allfilekeys)
// 	if err != nil {
// 		return nil, err
// 	}
...
// 	dskey := bytesToUUID(userlib.Argon2Key([]byte(password), salt, 16))
...
// 		println(err.Error())
// 	}
// 	allfilekeys.Files[filename] = filekeys //!!!

// 	// add updated allfilekeys to right location
// 	allfilekeys_marshal, _ = json.Marshal(allfilekeys)
// 	userlib.DatastoreSet(userdata.FKKey, allfilekeys_marshal)
...
// 	allfilekeys, err := userdata.GetAllFileKeys()
// 	if err != nil { return errors.New("Failed to get all file keys") }
// 	_, ok = allfilekeys.Files[filename]
