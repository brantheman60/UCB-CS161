...
// // convert small number (eg. 1, 2, 3) to letter string
// func itoa(i int) (a string) {
// 	return string(i+'a'-1)
// }
...
// 	newString := fileinfo_ID.String() + itoa(1) // deterministic; begin w/ 1 b/c file only has 1 part
...
// 	// receive original fileinfo from Datastore
// 	filekeys, ok := userdata.Files[filename]
// 	if !ok { return errors.New("File " + filename + " does not exist") }
...
// 	newString := filekeys.FileInfoID.String() + itoa(fileinfo.TotalParts + 1) //!!!
// 	newString_index := len(newString) - 16 // only use the last 16 bytes of newString
...
// 	fileinfo.TotalParts = fileinfo.TotalParts + 1
...
// 	newfileinfo_tag, err := userlib.HMACEval(filekeys.MKey, newfileinfo_enc)
// 	if err != nil { return errors.New("MAC Key doesn't work: Data is corrupted") }
// 	newfileinfo_cipher := append(newfileinfo_enc, newfileinfo_tag...)

// 	// store fileinfo back in original location
// 	userlib.DatastoreSet(filekeys.FileInfoID, newfileinfo_cipher)
// 	print("After appending, NumParts = ") //!!!
// 	println(fileinfo.TotalParts) //!!!
...
// 	// get filekeys from Datastore (for the original file)
// 	filekeys := userdata.Files[filename]
// 	// get fileinfo from filekeys (for the original file)
...
// 		newString := filekeys.FileInfoID.String() + itoa(i)
// 		newString_index := len(newString) - 16 // only use the last 16 bytes of newString
// 		file_ID, _ := uuid.FromBytes([]byte(newString)[newString_index:])
...
// 	filekeys, ok := userdata.Files[filename]
// 	if !ok { return "", errors.New("File doesn't exist")}
...
// 	IV := userlib.RandomBytes(16)
// 	userdata_encrypted := userlib.SymEnc(userdata.EncryptKey, IV, userdata_marshal)
// 	userdata_tag, err := userlib.HMACEval(userdata.HMACKey, userdata_encrypted)
// 	if err != nil { return "", errors.New("MAC Key doesn't work: Data is corrupted") }
// 	userdata_cipher := append(userdata_encrypted, userdata_tag...)
// 	userlib.DatastoreSet(userdata.DSKey, userdata_cipher)
...
// 	/** retrieve and check the fileinfo **/
// 	filekeys, ok := userdata.Files[filename]
// 	if !ok { return errors.New("File doesn't exist") }
...
// 	// check HMAC
// 	if len(mkey) != 16 { return errors.New("MAC Key doesn't work: Data is corrupted") }
// 	tag, err := userlib.HMACEval(mkey, fileinfo_enc)
// 	if err != nil { return errors.New("MAC Key doesn't work: Data is corrupted") }
// 	if !userlib.HMACEqual(fileinfo_tag, tag) {
// 		return errors.New("HMAC doesn't match: Data is corrupted")
// 	}
...
// 	users_arr := userdata.SharedUsers[filename]
// 	was_shared := false
// 	var user_index int
...
// 	userdata.SharedUsers[filename] = users_arr
